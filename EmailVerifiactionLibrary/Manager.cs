...
namespace EmailVerifiactionLibrary
{
    public class Manager
    {
        ...
        public void StartProcess()
        {
            ...
            bool _flag = false;
            _emailList = ReadExcelData(@"C:\Users\Rizwan\Downloads\2ndLot.xlsx");
            ...
            foreach (string s in _domain)
            {
                ...
                if (!_flag)
                {
                    var _invaliddomainemail = _emailList.Where(x => x.EmailAddress.Split('@').GetValue(1).ToString() == s).ToList();
                ...
                else
                {
                    ...
                    if (!_flag)
                    {
                        var _invalidmxrecordemail = _emailList.Where(x => x.EmailAddress.Split('@').GetValue(1).ToString() == s).ToList();
                    ...
                    {
                        var _formatemail = _emailList.Where(x => x.EmailAddress.Split('@').GetValue(1).ToString() == s).ToList();
                        foreach (Email e in _formatemail)
                        {
                            if (IsValidEmail(e.EmailAddress))
                            {
                                ...
                                e.Verification = Enum.VerificationStatus.EmailVerified.ToString();
                            }
                            else
                            {
                                e.VerificationStatusId = Enum.VerificationStatus.InvalidFormat;
                                e.Verification = Enum.VerificationStatus.InvalidFormat.ToString();
                            }
                            if (!_finalemaillist.Any(x => x.EmailAddress == e.EmailAddress))
                            {
                                _finalemaillist.Add(e);
        ...
        public static string SaveToExcel(List<Email> _emailList)
        {
            ...
            try
            {
                ...
                string filepath = @"C:\Users\Rizwan\Downloads\2ndLot-Generated.xlsx";
